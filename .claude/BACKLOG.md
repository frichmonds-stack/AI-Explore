...
- [ ] **Visualise the improvement model** (raised 2026-07-21) — the hero asset for the approach/premise page: `Stage · Context · Catalyst` (see DECISIONS → 2026-07-21 later). Owner wants it beautiful and elegant, so treat it as a design job, not a quick diagram. **Sequence it after the stages are settled** — a polished diagram makes the model look more finished than it is, and anchors wording that is still a sketch. Constraints already established: (a) *not* a ring — a circle flattens the sequence and nothing can hang beneath it, which kills the depth axis; (b) loop rather than spiral is fine, because catalysts carry the improvement claim so the geometry doesn't have to; (c) two axes — the loop reads across, depth reads down; (d) cycle diagrams are a crowded genre (Kolb, plan-do-review, PDSA), so the differentiation is in the sourcing and the honesty, never the shape; (e) depth collapsed by default, opened when you're stuck — which is the site's existing progressive-disclosure pattern, so an interactive version may beat a static one.
