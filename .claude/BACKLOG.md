...
- [ ] **SEO — bigger fix (decision needed)** — `usePageMeta` now sets per-page title/description/OG client-side (helps Google + gives structure), and `index.html` carries baseline OG tags. BUT `HashRouter` means non-JS crawlers (social-card bots: FB/LinkedIn/Slack/iMessage) still only see index.html. Proper fix = switch `HashRouter → BrowserRouter` + a build-time **pre-render/SSG** (e.g. `vite-react-ssg` or a prerender step) so every route ships real static HTML with its own meta/share card — works on any static host (Netlify/Vercel/Cloudflare). Coupled to the hosting choice; do once a host is picked. Retrofit `usePageMeta` onto the other pages (home/tools/guides/glossary/tracks) too — currently only on Articles.
