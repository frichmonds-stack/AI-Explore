...
- [x] **Save & Share (2026-07-01)** — bookmarks + share, no backend/accounts/tracking. `lib/useBookmarks.js` (localStorage pub-sub store, cross-tab sync), `SaveButton`, `ShareButton` (native share sheet → copy-link/email/X/LinkedIn intent URLs, no SDKs), `SavedPage` (`/saved`, grouped by type, clear-all), nav bookmark icon with live count, footer link. Save+Share wired onto tool/guide/article detail pages. **TODO (nice-to-have):** icon-only save on tool/guide/article *cards* (skipped on ToolCard to avoid clashing with its stretched select target — revisit).
